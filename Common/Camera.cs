...
namespace Common
{
	public class Camera
	{
		...
		{
			float sprintFactor = InputTracker.GetKey(Keycode.LCTRL)
				? 0.1f
				: InputTracker.GetKey(Keycode.LSHIFT)
					? 2.5f
					: 1f;
			Vector3 motionDir = Vector3.Zero;
			if (InputTracker.GetKey(Keycode.a))
			{
				motionDir += -Vector3.UnitX;
			}
			if (InputTracker.GetKey(Keycode.d))
			{
				motionDir += Vector3.UnitX;
			}
			if (InputTracker.GetKey(Keycode.w))
			{
				motionDir += -Vector3.UnitZ;
			}
			if (InputTracker.GetKey(Keycode.s))
			{
				motionDir += Vector3.UnitZ;
			}
			if (InputTracker.GetKey(Keycode.q))
			{
				motionDir += -Vector3.UnitY;
			}
			if (InputTracker.GetKey(Keycode.e))
			{
				motionDir += Vector3.UnitY;
			}
