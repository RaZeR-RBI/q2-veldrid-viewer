...
namespace MD2Viewer
{
	...
	public class MD2Reader
	{
		...
		public MD2Reader(MD2File file, IArrayAllocator allocator) =>
			(File, _allocator) = (file, allocator);

		public IEnumerable<MD2Frame> GetFrames() => File.Frames;

		public void ProcessFrame(MD2Frame frame, MD2FrameCallback callback)
		{
			var totalVertices = File.Triangles.Length * 3;
			var backingArray = _allocator.Rent<VertexNT>(totalVertices);
			...
			for (var i = 0; i < File.Triangles.Length; i++)
			{
				...
				var v1 = new VertexNT();
				var vf1 = frame.Vertices.Span[tri.VertexID1];
				...
				var v2 = new VertexNT();
				var vf2 = frame.Vertices.Span[tri.VertexID2];
				...
				var v3 = new VertexNT();
				var vf3 = frame.Vertices.Span[tri.VertexID3];
			...
			callback(frame.Name, backingArray.AsSpan().Slice(0, totalVertices));
			_allocator.Return(backingArray);
		}
	}
}